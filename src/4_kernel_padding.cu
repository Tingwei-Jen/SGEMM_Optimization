...
__global__ void sgemm_padding_kernel(float *A, float *B, float *C, int M, int N, int K, float alpha, float beta) {
...
void sgemm_padding(float *A, float *B, float *C, int M, int N, int K, float alpha, float beta) {
    ...
    sgemm_padding_kernel<<<dimGrid, dimBlock>>>(A, B, C, M, N, K, alpha, beta);
}