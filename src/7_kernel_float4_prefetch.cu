...
__global__ void sgemm_float4_prefetch_kernel(float *A, float *B, float *C, int M, int N, int K, float alpha, float beta) {
    ...
    __shared__ float sA_trans[2][BK][TILE_SIZE];
    __shared__ float sB[2][BK][TILE_SIZE];
    ...
    // Load the first tile into shared memory
    ...
    for (int i = 0; i < TILE_SIZE; i += sA_stride_size) {
        ...
        sA_trans[current][colIdx_sA * 4 + 0][i + rowIdx_sA] = temp.x;
        sA_trans[current][colIdx_sA * 4 + 1][i + rowIdx_sA] = temp.y;
        sA_trans[current][colIdx_sA * 4 + 2][i + rowIdx_sA] = temp.z;
        sA_trans[current][colIdx_sA * 4 + 3][i + rowIdx_sA] = temp.w;
    }

    // load B to shared memory
    #pragma unroll
    for (int i = 0; i < BK; i += sB_stride_size) {
        reinterpret_cast<float4 *>(&sB[current][i + rowIdx_sB][colIdx_sB * 4])[0] 
    ...
    for (int bk = BK; bk < K; bk += BK) {
        ...
        for (int i = 0; i < TILE_SIZE; i += sA_stride_size) {
            float4 temp = reinterpret_cast<float4 *>(&A[(i + rowIdx_sA) * K + colIdx_sA * 4])[0];
            sA_trans[next][colIdx_sA * 4 + 0][i + rowIdx_sA] = temp.x;
            sA_trans[next][colIdx_sA * 4 + 1][i + rowIdx_sA] = temp.y;
            sA_trans[next][colIdx_sA * 4 + 2][i + rowIdx_sA] = temp.z;
            sA_trans[next][colIdx_sA * 4 + 3][i + rowIdx_sA] = temp.w;
        }

        // load B to shared memory
        #pragma unroll
        for (int i = 0; i < BK; i += sB_stride_size) {
            reinterpret_cast<float4 *>(&sB[next][i + rowIdx_sB][colIdx_sB * 4])[0] 
                = reinterpret_cast<float4 *>(&B[(i + rowIdx_sB) * N + colIdx_sB * 4])[0];
        }
        ...
        for (int e = 0; e < BK; ++e) {
            ...
            for (int i = 0; i < TM; i+=4) {    // row
                reinterpret_cast<float4 *>(&avalue[i])[0] 
                    = reinterpret_cast<float4 *>(&sA_trans[current][e][rowIdx_smallTile + i])[0];
            }

            #pragma unroll
            for (int i = 0; i < TN; i+=4) {    // col
                reinterpret_cast<float4 *>(&bvalue[i])[0] 
                    = reinterpret_cast<float4 *>(&sB[current][e][colIdx_smallTile + i])[0];
            }
        ...
        // Move to the next tile (next becomes current)
        current = next;
    }

    // Perform the last matrix multiplication using the final tile
    // calculate Cvalue
    #pragma unroll
    for (int e = 0; e < BK; ++e) {
...
void sgemm_float4_prefetch(float *A, float *B, float *C, int M, int N, int K, float alpha, float beta) {
    ...
    sgemm_float4_prefetch_kernel<<<dimGrid, dimBlock>>>(A, B, C, M, N, K, alpha, beta);
}